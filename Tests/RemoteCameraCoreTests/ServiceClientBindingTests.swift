...
struct CameraHubControllerBindingTests {
  ...
  func routeCommand() async throws {
    ...
    let sut = StateServiceClientBinding(client: controller, service: hub)
  ...
  func routeState() async throws {
    ...
    var sut: StateServiceClientBinding<MockHub, MockController>?
    ...
    try await confirmation("State update", expectedCount: 3) { called in
      ...
      sut = StateServiceClientBinding(client: controller, service: hub)
...
final class MockCapture: CaptureServicePort {
  var state: RemoteCameraCore.CaptureServiceState { RemoteCameraCore.CaptureServiceState() }

  var onState: any Publisher<RemoteCameraCore.CaptureServiceState, any Error> {
    PassthroughSubject<RemoteCameraCore.CaptureServiceState, any Error>()
  }

  var onEvent: any Publisher<RemoteCameraCore.CaptureServiceEvent, any Error> {
    PassthroughSubject<RemoteCameraCore.CaptureServiceEvent, any Error>()
  }

  var onCapturedBuffer: any Publisher<RemoteCameraCore.BufferWrapper, Never> {
    PassthroughSubject<RemoteCameraCore.BufferWrapper, Never>()
  }

  func perform(_ command: RemoteCameraCore.CaptureServiceCommand) async throws {
  }
